...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.media:
            downloaded_file_name = await ryoishin.download_media(
    ...
    if not len(fedList):
        for a in range(3):
            async with ryoishin.conversation("@MissRose_bot") as bot_conv:
    ...
    try:
        await ryoishin.send_message(chat, f"/start")
    except BaseException:
    ...
    for fed in fedList:
        if Config.EXCLUDE_FED and fed in excludeFed:
            await ryoishin.send_message(chat, f"{fed} Excluded.")
            exCount += 1
            continue
        await ryoishin.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await ryoishin.send_message(chat, f"/fban {FBAN} {REASON}")
        await asyncio.sleep(3)
    await event.edit(
...
async def _(event):
    ...
    for fed in fedList:
        await ryoishin.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(5)
        await ryoishin.send_message(chat, f"/unfban {FBAN}")
        await asyncio.sleep(5)
