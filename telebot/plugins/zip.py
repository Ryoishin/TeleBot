...
@ryoishin.on(admin_cmd(pattern="zip"))
@ryoishin.on(sudo_cmd(pattern="zip", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@ryoishin.on(admin_cmd(pattern="unzip"))
@ryoishin.on(sudo_cmd(pattern="unzip", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mone = await eor(event, "Processing ...")
