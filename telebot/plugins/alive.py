...
async def amireallyalive(alive):
    ...
    if ALV_PIC:
        tele = f"**Welcome To Ryoishin **\n\n"
        tele += f"`{CUSTOM_ALIVE}`\n\n"
        tele += (
            f"{telemoji} **Telethon version**: `1.17`\n{telemoji} **Python**: `3.8.3`\n"
        )
        tele += f"{telemoji} **Ryoishin Version**: `{telever}`\n"
        tele += f"{telemoji} **More Info**: @RyoishinSupport\n"
        tele += f"{telemoji} **Sudo** : `{sudo}`\n"
        tele += f"{telemoji} **Ryoishin Uptime**: `{uptime}`\n"
    ...
    with BytesIO() as sticker:
        ...
        await borg.send_message(
            alive.chat_id,
            f"**Welcome To Ryoishin **\n\n"
            f"`{CUSTOM_ALIVE}`\n\n"
            f"{telemoji} **Telethon version**: `1.17`\n{telemoji} **Python**: `3.8.3`\n"
            f"{telemoji} **Ryoishin Version**: `{telever}`\n"
            f"{telemoji} **More Info**: @RyoishinSupport\n"
